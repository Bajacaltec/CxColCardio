...
def modificar():
    ...
    edad_modificar=st.number_input('Edad',1,300,fa[1])
    ...
    con.close()
    con = sqlite3.connect('Basededatos.db')
    cur = con.cursor()    #Problema no actualiza los registros si utilizo number input o text input, solo lo hace con los selectbox ????
    ...
    if insertar_datos==True:
            cur.execute("UPDATE cxcolcardio SET Nombre=?,Edad=?,NSS=?,Diagnostico=?,Genero=?,Fecha=?,Captura=?,hospital=? WHERE Nombre=?",(nom_modificar,edad_modificar,nss_modificar,dx_modificar,gen_modificar,fecha_modificar,captura_modificar,hospital_modificar,nom_base))
